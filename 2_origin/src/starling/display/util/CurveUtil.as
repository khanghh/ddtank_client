package starling.display.util
{
   public class CurveUtil
   {
      ...
      public static function quadraticCurve(a1x:Number, a1y:Number, cx:Number, cy:Number, a2x:Number, a2y:Number, error:Number = 0.75) : Vector.<Number>
      {
         _subSteps = 0;
         _bezierError = error;
         _terms[0] = a1x;
         ...
         subdivideQuadratic(0,0.5,0,output);
         subdivideQuadratic(0.5,1,0,output);
         return output;
      }
      
      public static function cubicCurve(a1x:Number, a1y:Number, c1x:Number, c1y:Number, c2x:Number, c2y:Number, a2x:Number, a2y:Number, error:Number = 0.75) : Vector.<Number>
      {
         _subSteps = 0;
         _bezierError = error;
         _terms[0] = a1x;
         ...
         _terms[7] = a2y;
         var output:Vector.<Number> = new Vector.<Number>();
         subdivideCubic(0,0.5,0,output);
         subdivideCubic(0.5,1,0,output);
         return output;
      }
      ...
      private static function quadratic(t:Number, axis:int) : Number
      {
         ...
         return oneMinusT * oneMinusT * a1 + 2 * oneMinusT * t * c + t * t * a2;
      }
      ...
      private static function cubic(t:Number, axis:int) : Number
      {
         ...
         return oneMinusT * oneMinusT * oneMinusT * a1 + 3 * oneMinusT * oneMinusT * t * c1 + 3 * oneMinusT * t * t * c2 + t * t * t * a2;
      }
