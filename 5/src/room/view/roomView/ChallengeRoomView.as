package room.view.roomView
{
    ...
    public class ChallengeRoomView extends BaseRoomView implements Disposeable
    {
        ...
        {
            _startGameTimer = new Timer(1000);
            super(info);
        }
        ...
        override protected function initEvents() : void
        {
            ...
            RutExpManager.Instance.addEventListener(RutExpManager.RUT_EXP_TOGGLED, __rutExpToggled);
        ...
        override protected function removeEvents() : void
        {
            ...
            RutExpManager.Instance.removeEventListener(RutExpManager.RUT_EXP_TOGGLED, __rutExpToggled);
        }
        ...
        // modified
        private function __rutExpToggled(e:Event) : void
        {
            var listPlayer:Array = roomInfo.players.list;
            var i:int = 0;
            var player:RoomPlayer = null;
            while (i < listPlayer.length)
            {
                player = listPlayer[i] as RoomPlayer;
                if (player.isHost)
                {
                    RutExpManager.Instance.setHostId(player.playerInfo.ID);
                    if (player.isSelf)
                    {
                        RutExpManager.Instance.IsHost = true;
                    }
                }
                else
                {
                    RutExpManager.Instance.setGuestId(player.playerInfo.ID);
                }
                i++;
            }
            ChatManager.Instance.sysChatYellow("hostId: " + RutExpManager.Instance.HostId);
            ChatManager.Instance.sysChatYellow("guestId: " + RutExpManager.Instance.GuestId);
        }
        ...
        protected function __retryStartGame(event:TimerEvent):void
        {
            var i:int = 0;
            var listPlayer:Array = roomInfo.players.list;
            var friend:RoomPlayer = null;
            if (RutExpManager.Instance.IsRutExp)
            {
                if (roomInfo.selfRoomPlayer.isHost)
                {
                    while(i < listPlayer.length)
                    {
                        friend = roomInfo.players.list[i] as RoomPlayer;
                        if (!friend.isSelf && RutExpManager.Instance.GuestId != friend.playerInfo.ID)
                        {
                            GameInSocketOut.sendGameRoomKick(friend.playerInfo.ID);
                        }
                        i++;
                    }
                    if (roomInfo.isAllReady() && !roomInfo.started)
                    {
                    ...
                    {
                        if (!roomInfo.players.hasKey(RutExpManager.Instance.GuestId))
                            GameInSocketOut.sendInviteGame(RutExpManager.Instance.GuestId);
                    }
                }
