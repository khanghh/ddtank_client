package worldboss.view
{
   ...
   public class RankingPersonInfoItem extends Sprite implements Disposeable
   {
      ...
      private function _init() : void
      {
         if(_longBg)
         {
            _bg = ComponentFactory.Instance.creatComponentByStylename("worldBossAward.rankingItemBg");
         }
         else
         {
            _bg = ComponentFactory.Instance.creatComponentByStylename("worldboss.RankingItem.bg");
         }
         addChild(_bg);
         _num % 2 == 0?_bg.setFrame(1):_bg.setFrame(2);
         _bgLine = ComponentFactory.Instance.creatComponentByStylename("worldboss.ranking.ViewItemSeperators");
         addChild(_bgLine);
         _txtName = ComponentFactory.Instance.creat("worldBoss.ranking.name");
         addChild(_txtName);
         _txtDamage = ComponentFactory.Instance.creat("worldBoss.ranking.damage");
         addChild(_txtDamage);
         _ranking = ComponentFactory.Instance.creat("worldBoss.ranking.num");
         addChild(_ranking);
         setValue();
      }
