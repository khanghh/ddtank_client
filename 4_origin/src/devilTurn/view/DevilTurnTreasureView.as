package devilTurn.view
{
   ...
   public class DevilTurnTreasureView extends DevilTurnTreasureViewUI
   {
      ...
      override protected function initialize() : void
      {
         ...
         updateViewInfo();
         var info1:GoodTipInfo = new GoodTipInfo();
         info1.itemInfo = ItemManager.Instance.getTemplateById(EquipType.DEVIL_TURN_BEADLIST[0]);
         beadTips1.tipData = info1;
         var info2:GoodTipInfo = new GoodTipInfo();
         info2.itemInfo = ItemManager.Instance.getTemplateById(EquipType.DEVIL_TURN_BEADLIST[1]);
         beadTips2.tipData = info2;
         var info3:GoodTipInfo = new GoodTipInfo();
         info3.itemInfo = ItemManager.Instance.getTemplateById(EquipType.DEVIL_TURN_BEADLIST[2]);
         beadTips3.tipData = info3;
         var info4:GoodTipInfo = new GoodTipInfo();
         info4.itemInfo = ItemManager.Instance.getTemplateById(EquipType.DEVIL_TURN_BEADLIST[3]);
         beadTips4.tipData = info4;
         var info5:GoodTipInfo = new GoodTipInfo();
         info5.itemInfo = ItemManager.Instance.getTemplateById(EquipType.DEVIL_TURN_BEADLIST[4]);
         beadTips5.tipData = info5;
      ...
      private function __onTimer(e:TimerEvent) : void
      {
         var date:* = null;
         var dateStr:* = null;
         var time:int = _model.freeDate - TimeManager.Instance.NowTime() / 1000;
         if(time >= 0 && _model.freeTimes > 0)
         {
            date = DateUtils.shorTimeRemainArr(time);
            dateStr = date[1] + ":" + date[2];
            freeDateText.text = LanguageMgr.GetTranslation("tank.devilTurn.freeSacrificeTips",dateStr,_model.freeTimes,_totalFreeLotteryTimes);
         }
         else
         {
            _timer.stop();
      ...
      {
         var view:DevilTurnBox = item as DevilTurnBox;
         if(index < boxList.array.length)
         {
            view.info = boxList.array[index] as DevilTurnBoxItem;
         }
         else
         {
            view.info = null;
      ...
      {
         var info:* = null;
         var rank:Label = item.getChildByName("rank") as Label;
         var name:Label = item.getChildByName("name") as Label;
         var score:Label = item.getChildByName("score") as Label;
         if(index < rankList.array.length)
         {
            info = rankList.array[index] as DevilTurnRankInfo;
            rank.text = info.rank.toString();
            name.text = info.name;
            item.tipData = info.area;
            var _loc7_:* = true;
            item.mouseChildren = _loc7_;
            item.mouseEnabled = _loc7_;
      ...
      private function onRenderRankAwardList(item:Box, index:int) : void
      {
         ...
         var rank:Label = item.getChildByName("rank") as Label;
         var score:Label = item.getChildByName("score") as Label;
         if(index < rankAwardList.array.length)
         {
            info = rankAwardList.array[index] as DevilTurnRankRewardItem;
            rank.text = info.rank;
            var _loc6_:* = info.Desc;
            score.text = _loc6_;
            item.tipData = _loc6_;
         }
         else
      ...
      private function onOpenShopView() : void
      {
         ...
         var view:DevilTurnShopView = new DevilTurnShopView();
      ...
      private function onOpenBoxConversionView() : void
      {
         ...
         var view:DevilTurnBoxConvertView = new DevilTurnBoxConvertView();
         LayerManager.Instance.addToLayer(view,3,true,1);
      }
      ...
      private function onClickSelectRankList(index:int) : void
      {
         SoundManager.instance.playButtonSound();
         rankBg.index = index;
         rankList.visible = index == 0;
         rankAwardList.visible = index == 1;
      }
