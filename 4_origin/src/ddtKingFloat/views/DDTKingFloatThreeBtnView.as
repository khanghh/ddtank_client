package ddtKingFloat.views
{
   ...
   public class DDTKingFloatThreeBtnView extends Sprite implements Disposeable
   {
      ...
      public function DDTKingFloatThreeBtnView()
      {
         super();
         this.x = 885;
         this.y = 258;
      ...
      private function initView() : void
      {
         var i:int = 0;
         var tmp:* = null;
         _bg = ComponentFactory.Instance.creatBitmap("ddtKing.threeBtnBg");
         _leapBtn = ComponentFactory.Instance.creatComponentByStylename("ddtKing.leapBtn");
         _leapBtn.tipData = LanguageMgr.GetTranslation("floatParade.game.leapBtnTipTxt");
         _invisibilityBtn = ComponentFactory.Instance.creatComponentByStylename("ddtKing.invisibilityBtn");
         _invisibilityBtn.tipData = LanguageMgr.GetTranslation("floatParade.game.invisibilityBtnTipTxt");
         _cleanBtn = ComponentFactory.Instance.creatComponentByStylename("ddtKing.cleanBtn");
         _cleanBtn.tipData = LanguageMgr.GetTranslation("floatParade.game.cleanBtnTipTxt");
         _missileBtn = ComponentFactory.Instance.creatComponentByStylename("ddtKing.missileBtn");
         ...
         for(i = 0; i < 4; )
         {
            tmp = ComponentFactory.Instance.creat("ddtKing.freeTipMc") as MovieClip;
            tmp.x = -36;
            tmp.y = -14 + 44 * i;
      ...
      private function initEvent() : void
      {
         ...
         DDTKingFloatManager.instance.addEventListener("floatParadeRefreshItemCount",refreshFreeCount);
      }
      
      private function refreshFreeCount(event:Event) : void
      {
         var i:int = 0;
         var tmp:Array = DDTKingFloatManager.instance.itemFreeCountList;
         for(i = 0; i < 4; )
         {
            if(tmp[i] > 0)
            {
      ...
      {
         var tmp:DDTKingFloatEvent = new DDTKingFloatEvent("floatParadeLeapPromptShowHide");
         tmp.data = {"isShow":false};
         DDTKingFloatManager.instance.dispatchEvent(tmp);
      }
      ...
      private function clickHandler(event:MouseEvent) : void
      {
         ...
         trace("selfPlayer.x==================" + DDTKingFloatManager.instance.selfPlayer.x);
         ...
         if(_leapBtn !== _loc4_)
         {
            if(_invisibilityBtn !== _loc4_)
            {
               if(_cleanBtn !== _loc4_)
               {
                  ...
                  {
                     _recordClickTag = 3;
                     missileFrame = ComponentFactory.Instance.creatComponentByStylename("ddtKing.missileFrame.frame");
      ...
      public function useSkill() : void
      {
         ...
         var tmpObj:Object = DDTKingFloatManager.instance.getBuyRecordStatus(_recordClickTag + 2);
         var needMoney:int = DDTKingFloatManager.instance.dataInfo.useSkillNeedMoney[_recordClickTag];
      ...
      private function useSkillConfirm(evt:FrameEvent) : void
      {
         ...
         {
            needMoney = DDTKingFloatManager.instance.dataInfo.useSkillNeedMoney[_recordClickTag];
            ...
            {
               tmpObj = DDTKingFloatManager.instance.getBuyRecordStatus(_recordClickTag + 2);
      ...
      private function sendUseSkillSocket(type:int, isBand:Boolean, isFree:Boolean, otherId:int = 0, otherZone:int = -1) : void
      {
         ...
         else
         {
            ...
            arr.push(otherId);
            arr.push(otherZone);
            DDTKingFloatManager.instance.missileArgArr = arr;
      ...
      private function removeEvent() : void
      {
         ...
         DDTKingFloatManager.instance.removeEventListener("floatParadeRefreshItemCount",refreshFreeCount);
      }
