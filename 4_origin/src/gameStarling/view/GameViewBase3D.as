package gameStarling.view
{
   ...
   public class GameViewBase3D extends BaseStateView
   {
      ...
      {
         var roomType:int = RoomManager.Instance.current.type;
         if(roomType == 120 || roomType == 1 && RoomManager.Instance.current.gameMode == 120)
         {
      ...
      private function addPlayerHander(e:DictionaryEvent) : void
      {
         ...
         if(info is Player)
         {
            if(!info.movie)
            {
               movie = CharactoryFactory.createCharacter(info.playerInfo,"game3d");
               movie.show();
               info.movie = GameCharacter3D(movie);
            }
            if(!info.character)
            {
            ...
            player = new GamePlayer3D(info,info.character,GameCharacter3D(movie));
      ...
      protected function setupGameData() : void
      {
         ...
         for each(var info in _gameInfo.livings)
         {
            if(info is Player)
            {
               ...
               if(!p.movie)
               {
                  movie = CharactoryFactory.createCharacter(p.playerInfo,"game3d");
                  movie.show();
                  p.movie = GameCharacter3D(movie);
               }
               if(!p.character)
               {
               ...
               if(p.isSelf)
               {
                  view = new GameLocalPlayer3D(_gameInfo.selfGamePlayer,p.character as ShowCharacter,p.movie as GameCharacter3D,p.templeId);
                  _selfGamePlayer = view as GameLocalPlayer3D;
               }
               else
               {
                  view = new GamePlayer3D(p,p.character as ShowCharacter,p.movie as GameCharacter3D,p.templeId);
               }
               if(p.movie)
               {
      ...
      {
         var player:GamePlayer3D = event.data as GamePlayer3D;
         player.playRingSkill();
      }
      
      protected function __onAddGuardCoreAnimation(e:GameEvent) : void
      {
         var player:GamePlayer3D = e.data as GamePlayer3D;
      ...
      private function removeGameData() : void
      {
         ...
         {
            StarlingObjectUtils.removeChildAllChildren(view,false);
            StarlingObjectUtils.disposeObject(view);
         }
         _players.clear();
         _players = null;
         ...
         for each(var liv in _allLivings)
         {
            ...
            if(!(liv.isHidden || liv.team == GameControl.Instance.Current.selfGamePlayer.team || !liv.isLiving || liv.LivingID == _self.LivingID))
            {
               ...
               if(_self.isLiving && _self.isAttacking)
               {
                  ...
                  _gameLiving = _map.getPhysical(liv.LivingID) as GameLiving3D;
