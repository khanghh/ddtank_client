...
//			LayerManager.Instance.addToLayer(_ddtConsole,4,false,0,false);
		...
		public function handle(param1:String) : void
		{
			...
			try
			{
				...
				if (cmd == "#self")
				{
					ChatManager.Instance.sysChatYellow("selfId: " + RoomManager.Instance.current.selfRoomPlayer.playerInfo.ID);
				}
				else if (cmd == "#rut")
				{
                    GameInSocketOut.sendCreateRoom("rutexp", 1, 2, "", false);
                    GameInSocketOut.sendExitScene();
                    GameInSocketOut.sendInviteGame(RutExpManager.Instance.GuestId);
                    SocketManager.Instance.out.sendPlayerExit(RutExpManager.Instance.HostId);
				}
				else if (cmd == "#connect")
				{
